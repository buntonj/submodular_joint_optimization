...
function [x,val] = fnnqp(Q,p,x_prev,tol)
if size(Q) == 0
    val = 0;
    x = [];
    return
...
n = size(p,1);
x = x_prev;
I = x > 0; %inactive constraints (i.e. x > 0)
A = ~I; %active constraints  (i.e. x=0)
w  = -p - Q*x;
s = zeros(n,1);
% intial loop for a start guess
iter = 0;
iter_max = 30*n;
s(I) = -Q(I,I)\ p(I);
s(A) = 0;
