...
function [A,subopt,costs] = bootleg_min_norm_point(F,V, opt) 
if ~exist('opt','var')
    opt = sfo_opt;
end
...
while 1
    ...
    costs = [costs, Fbest];
    subopt = [subopt, Fbest-sum(xhat(xhat<0))];
    ...
    if abs(xhat'*phat - xhat'*xhat)<TOL || (subopt(i)<eps)
...
%if sfo_opt_get(opt,'verbosity_level',0)>0
%    disp(sprintf('suboptimality bound: %f <= min_A F(A) <= F(A_best) = %f; delta<=%f \n',Fbest-subopt(i),Fbest,subopt(i)));
end
